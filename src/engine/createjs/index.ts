...
export default class Player extends createjs.Container {
    ...
    constructor ( container, options ) {
        ...
        if ( container && createjs ) {
            ...
            !width ? width = canvas.width : void 0;
            !height ? height = canvas.height : void 0;

            this.setFrame( 0, 0, width, height );
            this.setContentMode( 'AspectFill' );
    ...
    _doStart ( range, reverse, fromFrame ) {
        ...
        this._animator.onUpdate = ( value ) => {
            ...
            if ( typeof this._onPercentage === 'function' ) {
                this._onPercentage( parseFloat( ( this._currentFrame + 1 ).toString( ) ) / parseFloat( this._videoItem.frames ) );
            }
        }
    ...
    _resize ( ) {
        ...
        this.transformMatrix = new createjs.Matrix2D( scaleX, 0.0, 0.0, scaleY, this._frame.x + translateX, this._frame.y + translateY );
    ...
    _updateMask ( ) {
        if ( this._clipsToBounds ) {
            ...
            if ( this.mask.__width !== this._frame.width || this.mask.height !== this._frame.height ) {
    ...
    _update ( ) {
        if ( this._videoItem === undefined) { return void 0; }
        this._resize( );
        this._updateMask( );
